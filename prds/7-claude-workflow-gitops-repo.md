...
- [x] Add CodeRabbit MCP server to `.mcp.json` (npx coderabbitai-mcp with GITHUB_TOKEN)
- [x] Run `/make-autonomous` in Thomas's repo to install YOLO skill symlinks and SessionStart hook
- [x] Ensure `.claude/skills/` symlinks are covered by `.git/info/exclude` (already under `.claude/`)
- [x] Ensure `.claude/settings.local.json` permissions and hooks from `/make-autonomous` are invisible (already auto-local)
- [x] Verify CodeRabbit CLI runs during pre-push hook
- [x] Verify PRD skills work for creating/tracking work in Thomas's repo
- [x] Verify `/clear` → auto-resume loop works in Thomas's repo
